...
export function buildReviewerPrompt(
  ...
  const sections = [
    ...
    `1. ${diffInstruction}`,
    `2. STRICT READ-ONLY RULE: Do NOT edit, write, create, or delete any source files. NEVER run project code, test runners, or package managers (node, npm, npx, python, pytest, etc.) — executing them hangs the review process in non-interactive mode and the review is killed with no verdict (P-075: three consecutive vendor kills, exit 124). Quick read-only git inspection (git diff, git status, git log) and reading files are the ONLY permitted actions; base your verdict on the diff and source, not on executing it.`,
    `2b. WORKSPACE CLEANLINESS: NEVER create any file inside the working repository — including logs, err*.txt/out*.txt captures, or scratch scripts. The working tree must be byte-identical to how you found it; any new file counts as a violation.`,
